...
```{r, echo=FALSE}
# setwd("analysis/waitlist-survival/")
```
...
```{r plot_data}
# Univariate plot showing relationship between predictor and outcome
# Creates n bins on x-axis. For numeric var, uses quantile binning 
# (ggplot2::cut_number()). Else forcats::fct_lump_n() to create n bins. 
# Adds an explicit bin for missing (NAs). 
# Uses posterior mean and 95% credible intervals from empirical bayes 
#  (Laplace smoothing) of probability. 


plot_data <- function(data, var, n = 12){
  # n is the maximum number of bins/levels to use in plot
  k = 5          # shrinkage prior (number of prior observations)
  ...
  tibble(x, y) %>% 
    ...
    summarize(
      ...
      alpha = (n1 + k*mu),
      beta =  (n-n1 + k*(1-mu)), 
      lower = qbeta(.025, alpha, beta), 
      upper = qbeta(.975, alpha, beta),
...
```{r univariate-smoother}
library(mgcv)
smoother <- function(x, y, method = "gam"){
  method = match.arg(method, c("gam", "glm"))
  missing = (is.na(x) | is.na(y))
  x = x[!missing] 
  y = y[!missing]
  if(n_distinct(x) == 1) return(NULL)
  if(is.character(x) || n_distinct(x) < 7) x = factor(x)
  bs = ifelse(is.factor(x), "re", "tp")
  if(method == "gam"){
  tibble(x, y) %>% 
    mgcv::gam(y~s(x, bs=bs, k = 8), # max of ~k edf
              method = "REML",
              family = binomial(),
              data = .)
    } else if(method == "glm"){
...
<!-- 3. Add size percentiles 

```{r, eval = FALSE, echo=FALSE}
calc_cdc_perc <- function(x, L, M, S){
  Z = ifelse(L != 0, ((x/M)^L - 1)/(L*S), log(x/M)/S) 
  pnorm(Z)
}


WEIGHT_KG_PERC = read_csv("~/Downloads/wtageinf.csv") %>% filter(Agemos < 24) %>% 
  bind_rows(read_csv("~/Downloads/wtage.csv")) %>% 
  filter(Agemos %in% seq(6.5, 500, by = 12)) %>% 
  transmute(
    GENDER = case_match(Sex, 1 ~ "M", 2~"F"), 
    AGE = floor(Agemos/12),
    percentile = pmap(list(L, M, S), function(L, M, S){
      function(x) calc_cdc_perc(x, L, M, S)
      })
  )

HEIGHT_CM_PERC = read_csv("~/Downloads/lenageinf.csv") %>% 
  mutate_if(is.character, as.numeric) %>% 
  filter(Agemos < 24) %>% 
  bind_rows(read_csv("~/Downloads/statage.csv")) %>% 
  filter(Agemos %in% seq(6.5, 500, by = 12)) %>% 
  transmute(
    GENDER = case_match(Sex, 1 ~ "M", 2~"F"), 
    AGE = floor(Agemos/12),
...
add_size_percentiles <- function(data){
  data %>% 
    left_join(WEIGHT_KG_PERC, by = c("AGE", "GENDER")) %>% 
    mutate(WEIGHT_KG_PERC = map2_dbl(percentile, WEIGHT_KG, ~.x(.y))) %>% 
    select(-percentile) %>% 
    left_join(HEIGHT_CM_PERC, by = c("AGE", "GENDER")) %>% 
    mutate(HEIGHT_CM_PERC = map2_dbl(percentile, HEIGHT_CM, ~.x(.y))) %>% 
    select(-percentile)
}



data_train = data_train %>% add_size_percentiles()
data_test = data_test %>% add_size_percentiles()
...
```{r echo=FALSE}
# auc    brier log_loss   accuracy
# 0.731 0.0839    0.296      0.9
# 0.727 0.0847    0.298    0.902  | No LISTING_CTR_CODE
# 0.727 0.0843    0.297    0.902  | removed DONCRIT_{AGE, WGT, HGT}
# 0.732 0.0841    0.296    0.902  | removed extra CAND_DIAG_LISTING
# 0.737 0.0839    0.294    0.902  | using DIAG_CODE
# 0.740 0.0840    0.294      0.9  | polynomials
# 0.730	0.083	    0.295	   0.900
# 0.731	0.084	0.295	0.899
```
...
```{r gam-fit-plots, echo=FALSE, eval=FALSE}
# #| out-width: "100%"
# library(gratia)
# library(patchwork)
# gratia::draw(fit_gam, scales="fixed", parametric = TRUE) & ylim(-3,3)
```


```{r gam-single-plot, eval=FALSE, echo=FALSE}
# gratia::draw(fit_gam, select = 1) + coord_cartesian(ylim = c(-2, 2))
```
...
#   iter = 5, 
#   initial = 5, 
#   param_info = dials::parameters(mtry(range=c(1, 100))),
#   metrics = metric_set(roc_auc, brier_class, mn_log_loss, accuracy), 
...
```{r, echo=FALSE, eval=FALSE}
# best_auc = tibble(trees = 75, tree_depth=1)
# set.seed(1234)
# bt_fit = finalize_workflow(bt_wflow, best_auc) %>% fit(data_train)
```
...
#         GAM = 1-predict(fit_gam, data_test_gam, type = "response") %>% as.numeric
#     ) %>%
#   mutate(across(3:6, logit)) %>% 
#   glm(outcome ~ lasso + xgboost + RF + GAM, 
...
TODO:
- Compare variable important
- compare partial dependence of all models
- Plots of decision threshold vs. decision metrics
- PDP/SHAP/LIME plots from all models
- Standard Variable importance (perhaps from PDP)
- Residual check against listing centers and regions
- residual check against other predictors not included in model
- Repeat this multiple times (nested cross-validation)
- Ensemble and checking consistent predictions across models
  ** Paper: does the predictive model choice change decisions? 
...
data_test %>% 
  ...
  pivot_longer(c(lasso, xgboost, RF, GAM, ensemble)) %>% 
  split(.$name) %>% 
  walk(~ cal_plot_breaks(., truth = outcome, estimate = value, num_breaks = 10) %>% print)
```
...
tmp = data_test %>% 
  ...
  pivot_longer(c(lasso, xgboost, RF, GAM)) %>% 
  group_by(WL_ID_CODE, outcome) %>% 
  summarize(mu= mean(value), sd=sd(value), range = diff(range(value)), 
            median =median(value)) %>% 
  ungroup() %>% 
  arrange(-sd) 

tmp %>% ggplot(aes(mu, fill = factor(outcome))) + geom_density(alpha = .25)
tmp %>% ggplot(aes(median, fill = factor(outcome))) + geom_density(alpha = .25)
...
```{r}
# Age and Gender adjusted size percentiles
size_perc = bind_rows(data_train, ) %>%  # data_test
  select(GENDER, AGE, HEIGHT_CM, WEIGHT_KG, BMI, outcome) %>% 
  group_by(GENDER, AGE) %>% 
  mutate(
    n = n(),
    HEIGHT_CM_PERC = percent_rank(HEIGHT_CM),
    WEIGHT_KG_PERC = percent_rank(WEIGHT_KG),
    ) %>% 
  arrange(n)
size_perc %>% 
  pivot_longer(c(-outcome, -n, -GENDER)) %>% 
  ggplot(aes(value, 1-outcome)) + geom_smooth() + facet_wrap(~name, scales=  "free_x") + 
  coord_cartesian(ylim = c(.8, 1))

size_perc %>% ggplot(aes(HEIGHT_CM, HEIGHT_CM_PERC, color=outcome)) + geom_point()


glm(outcome ~ AGE + BMI + HEIGHT_CM + HEIGHT_CM_PERC, # + #WEIGHT_KG_PERC, 
    family = binomial(), data = size_perc) %>% summary
...
```{r}
pdp_plot <- function(explainer, var){
  mu = 1-mean(data_test$outcome)
  set.seed(9876)
  df = model_profile(explainer, variable = var)$agr_profiles %>% 
    as_tibble %>% rename(x = "_x_", yhat = "_yhat_") %>% 
    mutate(yhat = yhat - mean(yhat) + mu) 
    
  if(is.factor(df$x) || is.character(df$x) || (n_distinct(df$x) < 4) ) {
    plt = df %>% 
    ggplot(aes(as.factor(x),yhat)) + 
    geom_errorbar(aes(ymin=yhat, ymax=yhat)) + 
    ...
    plt + 
      geom_hline(yintercept = mu, color = "orange", alpha = .5)  +
      geom_rug(sides = "b", alpha = .25) + 
      labs(x = var, y = "avg prediction")  +
      coord_cartesian(ylim = c(.825, .975))  
}


# pdp_plot(bt_explain, imp_vars[9])
  

# model_profile(bt_explain, variable = "AGE") %>% plot()
imp_vars = 
  bt_fit %>% extract_fit_parsnip() %>% 
  vip::vi() %>% filter(Importance > 1E-8) %>% 
  pull(Variable) %>% 
  str_remove("na_ind_") %>% #str_remove("_X.+") %>% 
  str_remove("_Unknown") %>% str_remove("_Yes") %>% 
  str_replace("ABO(_.+)", "ABO") %>% 
...
walk(imp_vars, \(var) pdp_plot(bt_explain, var = var) %>% plot)
...
```{r}
bind_rows(
vip::vi(cv_fit, lambda = cv_fit$lambda.min) %>% filter(Importance > 1E-8) %>% 
  transmute(model="lasso", Variable, rank = min_rank(-Importance))
,fit_gam %>% tidy() %>% arrange(p.value) %>% 
  transmute(model = "gam", Variable=term, rank = min_rank(p.value))
# ,rf_fit %>% extract_fit_parsnip() %>% vip::vi() %>% filter(Importance > 1E-8) %>% 
#   transmute(model="rf", Variable, rank = min_rank(-Importance))
,bt_fit %>% extract_fit_parsnip() %>% vip::vi() %>% filter(Importance > 1E-8) %>%
  transmute(model="bt", Variable, rank = min_rank(-Importance))
) %>% 
  mutate(Variable = Variable %>% 
           str_remove("s\\(") %>% str_remove("\\)") #%>%str_remove("_poly.+") 
  ) %>% 
  mutate(Var2 = get_raw_variable_names(Variable)) %>% 
  group_by(Var2, model) %>% 
    slice_min(rank) %>% 
  group_by(Var2) %>% 
  summarize(n_ranked = n(), med_rank = median(rank, na.rm=TRUE), min_rank = min(rank, na.rm=TRUE)) %>% 
  arrange(-n_ranked, med_rank) %>% print(n=100)
```



```{r}
fit_gam %>% tidy() %>% arrange(p.value) %>% print(n=30)
# 3 CAND_DAIG
# VENT
# HEIGHT poly(3)
# ECMO, DIALYSIS
...
# ...
# median_refusals (2)
# FUNC_STAT_CAND_REG (3.5)
# ABO (1.3)
# LIFE_SUPPORT_OTHER .5
# 
```
...
```{r}
set.seed(9876)
model_profile(bt_explain, variable = "eGFR")$agr_profiles %>% 
  as_tibble %>% rename(x = "_x_", yhat = "_yhat_")
```
...
```{r}
fit = smoother(data_train$eGFR, data_train$outcome)

tibble(x = seq(0, 200, length=100)) %>% 
  mutate(p = 1-predict(fit, ., type = "response")) %>% 
...
```{r}
data_train %>% plot_data("eGFR")
```


```{r}
data_train %>% plot_data("ECMO_CAND_REG")
```
...
```{r}
data_train %>% 
  ggplot(aes(eGFR, outcome)) + geom_smooth(method = "glm")
```



```{r}
data_train %>% plot_data("HEMODYNAMICS_CO")
```
