...
Best is trial #420/500 with value: 1.246 for log loss
...
```{r xgboost-model, warning=FALSE, message=FALSE}
#| echo: false
#| warning: false
#| message: false
...
Best is trial #133/500 with value: 2.58.
...
explainer = shap.TreeExplainer(one_hot_model_CD)
...
num_features = X_train_one_hot.select_dtypes(include=['number']).columns.tolist()
print("Numerical Features:", num_features)

# Get the indices of numerical features
num_feature_indices = [X_train_one_hot.columns.get_loc(col) for col in num_features]
...
```{r mean-absolute-shap-values, eval=TRUE}

features <- names(py$X_train_one_hot)

mean_abs_shap_values <- colMeans(abs(shap_df))  # Compute mean absolute SHAP values

```

```{r radar-chart, eval=TRUE}
...
```{r pos-neg-shap-values, eval=TRUE}
...
```{r radar-chart-positive-negative, eval=TRUE, warning=FALSE, message=FALSE}
