...
# Assuming `gain` and `loss` are pandas DataFrames with 'Feature' and 'Importance' columns
...
# Assuming `common_features_df` contains the common features
...
# Convert NaN values to a string in categorical columns
categorical_columns_filtered = [col for col in X_train_filtered.columns if X_train_filtered[col].dtype == 'object']
X_train_filtered[categorical_columns_filtered] = X_train_filtered[categorical_columns_filtered].fillna('missing')


# Convert NaN values to a string in categorical columns
categorical_columns_filtered = [col for col in X_test_filtered.columns if X_test_filtered[col].dtype == 'object']
X_test_filtered[categorical_columns_filtered] = X_test_filtered[categorical_columns_filtered].fillna('missing')
...
# NaN values in the column at index 15
has_nan = X_train_filtered.iloc[:, 15].isna().any()
print("Column at index 15 contains NaN values:", has_nan)
...
columns_to_convert = [15]  

for index in columns_to_convert:
    # Convert for training data
    if isinstance(index, int):
        column_name = X_train_filtered.columns[index]
    else:
        column_name = index

    X_train_filtered[column_name] = pd.to_numeric(X_train_filtered[column_name].replace('Missing', pd.NA), errors='coerce')
    X_test_filtered[column_name] = pd.to_numeric(X_test_filtered[column_name].replace('Missing', pd.NA), errors='coerce')

# Verify conversion
print(X_train_filtered.iloc[:, 15].head())
print(X_test_filtered.iloc[:, 15].head())
