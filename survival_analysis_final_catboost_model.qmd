...
full_model %<>%
  mutate(
    ...
    VAD_REg = factor(VAD_REg, levels = c(-1, 1), labels = c("No", "Yes")),
...
p_values <- c(
  ...
  p_value_cat(full_model$VAD_REg == "Yes", full_model$Outcome),
...
demo_table <- full_model %>%
  ...
  summarise(
    ...
    `VAD %` = paste(pct(VAD_REg == "Yes"), n_for_feature(VAD_REg)),
...
# Calculate feature importances for all methods
gain_feature_importance = model_auc.get_feature_importance(prettified=True)
loss_feature_importance = model_auc.get_feature_importance(test_pool, type='LossFunctionChange', prettified=True)
prediction_value_feature_importance = model_auc.get_feature_importance(test_pool, type='PredictionValuesChange', prettified=True)
...
# Create pandas dataframes for each method
gain_df = pd.DataFrame(gain_feature_importance)
loss_df = pd.DataFrame(loss_feature_importance)
prediction_value_df = pd.DataFrame(prediction_value_feature_importance)
...
# Ensure that 'Feature Id' is the same data type in all DataFrames
gain_df['Feature Id'] = gain_df['Feature Id'].astype(str)
loss_df['Feature Id'] = loss_df['Feature Id'].astype(str)
prediction_value_df['Feature Id'] = prediction_value_df['Feature Id'].astype(str)
shap_df['Feature Id'] = shap_df['Feature Id'].astype(str)


# Rename columns to prevent conflicts during merge
gain_df.rename(columns={'Importances': 'Importance_Gain'}, inplace=True)
loss_df.rename(columns={'Importances': 'Importance_Loss'}, inplace=True)
prediction_value_df.rename(columns={'Importances': 'Importance_PredictionValue'}, inplace=True)
shap_df.rename(columns={'Importances': 'Importance_SHAP'}, inplace=True)

# Merge dataframes on 'Feature Id' for comparison
comparison_df = gain_df.merge(loss_df[['Feature Id', 'Importance_Loss']], on='Feature Id')

comparison_df = comparison_df.merge(prediction_value_df[['Feature Id', 'Importance_PredictionValue']], on='Feature Id')

comparison_df = comparison_df.merge(shap_df[['Feature Id', 'Importance_SHAP', 'SHAP_Direction']], on='Feature Id')

# Rank features based on each importance metric
comparison_df['Rank_Gain'] = comparison_df['Importance_Gain'].rank(ascending=False)
comparison_df['Rank_Loss'] = comparison_df['Importance_Loss'].rank(ascending=False)
comparison_df['Rank_PredictionValue'] = comparison_df['Importance_PredictionValue'].rank(ascending=False)

# Rank SHAP values by absolute value and create the rank column for SHAP
comparison_df['Rank_SHAP'] = comparison_df['Importance_SHAP'].abs().rank(ascending=False)

# Sort by PredictionValuesChange first, then LossFunctionChange, SHAP (absolute value), and finally Gain
comparison_df = comparison_df.sort_values(
    by=['Importance_PredictionValue', 'Importance_Loss', 'Importance_SHAP', 'Importance_Gain'], 
    ascending=False
)
...
comparison_df <- py$comparison_df

# Convert the pandas dataframe to an R tibble
comparison_tbl <- as_tibble(comparison_df)
...
comparison_table <- comparison_tbl %>%
  ...
  select('Overall Rank', 'Feature Id', 
         'Importance_PredictionValue', 'Rank_PredictionValue',
         'Importance_Gain', 'Rank_Gain',
         'Importance_SHAP', 'SHAP_Direction', 'Rank_SHAP',
         'Importance_Loss', 'Rank_Loss') 
...
```{r echo=FALSE}
hybrid_cat <- cat_features[c(2,3,10)]
...
has_nan = hybrid_X_train.iloc[:, 39].isna().any()
print("Column at index 39 contains NaN values:", has_nan)
...
# Ensure the column at index 131 is categorical and "Missing" is added to categories
if 'Missing' not in hybrid_X_train.iloc[:, 39].cat.categories:
    hybrid_X_train.iloc[:, 39] = hybrid_X_train.iloc[:, 39].cat.add_categories(['Missing'])

# Explicitly cast the column to a category type to ensure compatibility
hybrid_X_train.iloc[:, 39] = hybrid_X_train.iloc[:, 39].astype('category')

# Replace NaN values with the "Missing" category
hybrid_X_train.iloc[:, 39] = hybrid_X_train.iloc[:, 39].fillna('Missing')
...
-   From Optuna Trial #40/50 with value: 0.74
...
feature_names = hybrid_model.feature_names_

# Calculate feature importances for all methods
gain_feature_importance = hybrid_model.get_feature_importance(prettified=True)
loss_feature_importance = hybrid_model.get_feature_importance(test_pool, type='LossFunctionChange', prettified=True)
prediction_value_feature_importance = hybrid_model.get_feature_importance(test_pool, type='PredictionValuesChange', prettified=True)
shap_feature_importance = hybrid_model.get_feature_importance(test_pool, type='ShapValues', prettified=True)
...
shap_df = pd.DataFrame({
    'Feature Id': feature_names,
    'Importances': abs(shap_feature_importance_df.values),
    'SHAP_Direction': shap_direction  # direction of SHAP values
})
...
```{r final-feature-importances, echo=FALSE}
...
# Create a formatted table using huxtable, including the ranks for each method and the 'Direction' column
comparison_table_final <- comparison_tbl %>%
  rowid_to_column(var = "Overall Rank") %>%
  select('Overall Rank', 'Feature Id', 
         'Importance_PredictionValue', 'Rank_PredictionValue',
         'Importance_Gain', 'Rank_Gain',
...
shap.plots.beeswarm(numerical_shap_values, max_display=20)
...
[SHAP Partial Dependence Plots](https://virginia.box.com/v/survival-analysis-shap-pdp)

```{r eval=FALSE, echo=FALSE}

feature_names <- py$feature_names
shap_data <- py$shap_feature_importance

pdp_shap_data <- shap_data %>% 
  select(1:31) 

colnames(pdp_shap_data) <- feature_names
...
# Function to create SHAP partial dependency plots for numerical features
create_pdp_numeric <- function(feature_name, test_data, pdp_shap_data, save_plots = TRUE) {
    ...
    if (!feature_name %in% colnames(test_data) || !feature_name %in% colnames(pdp_shap_data)){
    ...
    pdp_shap_data_feature <- data.frame(
      feature_value = test_data[[feature_name]],  # Original feature values
      shap_value = pdp_shap_data[[feature_name]]  # SHAP values
    )
    ...
    scatter_plot <- ggplot(pdp_shap_data_feature, aes(x = feature_value, y = shap_value)) +
      ...
      scale_x_continuous() +
      scale_y_continuous() +  
      labs(x = paste(feature_name, "Values"),
           y = "SHAP Values",
           title = paste("SHAP Partial Dependency Plot for", feature_name)) +
      theme_minimal()  # Apply a clean theme for the plot
    ...
    histogram_plot <- ggplot(pdp_shap_data_feature, aes(x = feature_value)) +
      ...
      theme_minimal() +
      labs(x = paste(feature_name, "Values"), y = "Count")
...
create_pdp_numeric(feature_to_plot, test_data, pdp_shap_data, save_plots = TRUE)
...
create_pdp_categorical <- function(feature_name, test_data, pdp_shap_data, save_plot = TRUE, plot_width = 5, plot_height = 4) {
  ...
  pdp_shap_data_feature <- data.frame(
    feature_value = test_data[[feature_name]],  # Actual feature values (categories)
    shap_value = pdp_shap_data[[feature_name]]  # Corresponding SHAP values
  )
  
  # Calculate the mean SHAP value for each category
  pdp_shap_summary <- pdp_shap_data_feature %>%
    group_by(feature_value) %>%
    summarize(mean_shap = mean(shap_value, na.rm = TRUE))
  
  plot <- ggplot(pdp_shap_summary, 
               aes(
                 x = if (feature_name %in% c("Diag_Code", "Status_Reg")) mean_shap else feature_value,
                 y = if (feature_name %in% c("Diag_Code", "Status_Reg")) feature_value else mean_shap,
                 fill = feature_value
               )) +
  geom_bar(stat = "identity", show.legend = FALSE) +
  
  # Conditionally set x and y axis labels based on whether the feature is Diag_Code or Status_Reg
  labs(
    x = if (feature_name %in% c("Diag_Code", "Status_Reg")) "Mean SHAP Value" else paste(feature_name, "Categories"),
    y = if (feature_name %in% c("Diag_Code", "Status_Reg")) paste(feature_name, "Categories") else "Mean SHAP Value",
    title = paste("Partial Dependency Plot for", feature_name)
  ) +
  
  # Apply conditional theme to hide the x-axis for "List_Ctr"
  theme_minimal() +
  theme(
    panel.grid = element_blank(),  # Remove gridlines if desired
    
    # Conditionally hide x-axis text, title, and ticks for "List_Ctr"
    axis.text.x = if (feature_name == "List_Ctr") element_blank() else element_text(),
    axis.title.x = if (feature_name == "List_Ctr") element_blank() else element_text(),
    axis.ticks.x = if (feature_name == "List_Ctr") element_blank() else element_line(),
    
    # Keep y-axis text, title, and ticks unchanged
...
```{r eval=FALSE}
create_pdp_categorical("Race", test_data, pdp_shap_data)
```
...
create_shap_pdp_plots <- function(feature_names, test_data, pdp_shap_data, save_plots = TRUE) {
  
  # Loop over each feature in the feature_names list
  for (feature_name in feature_names) {
    
    # Check if the feature is present in test_data
    if (!feature_name %in% colnames(test_data)) {
    ...
    if (is.numeric(test_data[[feature_name]])) {
      # If numeric, call the numeric plotting function
      create_pdp_numeric(feature_name, test_data, pdp_shap_data, save_plots = TRUE)
    } else if (is.factor(test_data[[feature_name]]) || is.character(test_data[[feature_name]])) {
      # If categorical, call the categorical plotting function
      create_pdp_categorical(feature_name, test_data, pdp_shap_data, save_plot = TRUE, plot_width = 5, plot_height = 4)
    } else {
...
```{r eval=FALSE}
create_shap_pdp_plots(feature_names, test_data, pdp_shap_data, save_plots = TRUE)
```
