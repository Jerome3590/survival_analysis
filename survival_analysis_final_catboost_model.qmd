...
demo_table <- full_model %>%
  group_by(Outcome) %>%
  summarise(
    `Gender (% Female)` = pct(Gender == "Female"),
    Age = med_iqr(Age),
    Weight = med_iqr(Weight),
    Height = med_iqr(Height),
    BMI = med_iqr(BMI),
    `Blood Type A` = pct(Blood_Type == "A"),
    `Blood Type B` = pct(Blood_Type == "B"),
    `Blood Type AB` = pct(Blood_Type == "AB"),
    `Blood Type O` = pct(Blood_Type == "O"),
    `Race White` = pct(Race == "White"),
    `Race Black` = pct(Race == "Black"),
    `Race Other` = pct(Race == "Other"),
    `VAD %` = pct(VAD_REg == "Yes"),
    eGFR = med_iqr(eGFR),
    Albumin = med_iqr(Albumin),
    `Dialysis %` = pct(Dialysis == "Yes"),
    `Ventilator %` = pct(Vent_Reg == "Yes"),
    `ECMO %` = pct(ECMO_Reg == "Yes")
  ) %>%
  t() %>%
  as.data.frame() %>%
...
# Rename columns
colnames(demo_table) <- c("Trait", "Transplanted", "WL Mortality")
...
[SHAP Partial Dependence Plots](https://virginia.box.com/v/dependence-plots)

```{python partial-dependence-plots, eval=FALSE, echo=FALSE}
...
def filter_valid_indices(shap_values, input_data):
    ...
    for col_idx in range(input_data.shape[1]):
        ...
        # Check if column is numeric and then apply filtering for NaN or inf values
        if np.issubdtype(col_data.dtype, np.number):  # For numeric columns
...
filtered_shap_values_clean = shap.Explanation(
    ...
    base_values=shap_values.base_values[valid_indices],
