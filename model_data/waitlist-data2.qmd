...
TODO:
1. Combine candidate data with waitlist ranges to see if wide/narrow offer consideration impacts survival (e.g., only consider donors < 200mi)

Changes:
1. Added Body Surface Area (BSA)
2. Updated Center Stats Calculations
...
- TODO: consider calculating relative sizes from growth charts (e.g., <https://www.cdc.gov/growthcharts/clinical_charts.htm>). This will help identify over or undersized candidates
...
- ~~Transfusions `TRANSFUSIONS_CAND`~~ This is collected at transplant
```
...
<!--
Outcome using the Diagnosis from *Registration*:
```{r, eval=FALSE, echo=FALSE}
tmp %>% 
  count(CAND_DIAG_CODE_REG, outcome) %>% 
  spread(outcome, n, fill=0) %>% 
  mutate(p = (`0` + .9*5) / (`0` + `1` + 5)) %>% 
  arrange(p) %>% 
  print_table()
